...
async function addSavedDate(req, res) {
	const { eventName, eventColor, eventContent, eventDate, userId } = req.body;
	const event = await new Event({ eventName, eventContent });
	try {
		const user = await User.findById({ _id: userId });
		const currentEvent = user.savedDates.get(eventDate);
		if (currentEvent) {
			user.savedDates.set(eventDate, {
				events: [event, ...currentEvent.events],
			});
		} else {
			user.savedDates.set(eventDate, { events: [event] });
		}
		await user.save();
		const eventsArray = user.savedDates.entries();
		const eventsObject = Object.fromEntries(eventsArray);
		res.status(200).send({ eventDate, events: eventsObject[eventDate].events });
	} catch (err) {
		console.log(err);
	}
}
